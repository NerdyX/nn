...
import { $, noSerialize, implicit$FirstArg, component$, useVisibleTask$, Slot, useStore } from "@builder.io/qwik";
...
function formActionQrl(action, arg2) {
  return globalActionQrl($(async (jsonData, event) => {
    ...
    const values = type === "application/x-www-form-urlencoded" || type === "multipart/form-data" ? decode(event.sharedMap.get("@actionFormData"), formDataInfo, ({ output }) => output instanceof Blob ? noSerialize(output) : output) : jsonData;
    ...
    if (!Object.keys(errors).length) {
      try {
        ...
        if (error instanceof AbortMessage || isDev && (error?.constructor?.name === "AbortMessage" || error?.constructor?.name === "RedirectMessage")) {
          throw error;
        } else {
          console.error(error);
...
const formAction$ = implicit$FirstArg(formActionQrl);
function valiFieldQrl(schema) {
  return $(async (value2) => {
    const resolvedSchema = await schema.resolve();
    const result = await safeParseAsync(typeof resolvedSchema === "function" ? resolvedSchema() : resolvedSchema, value2, {
      abortPipeEarly: true
    });
    return result.issues?.[0].message || "";
  });
}
const valiField$ = implicit$FirstArg(valiFieldQrl);
function valiFormQrl(schema) {
  return $(async (values) => {
    const resolvedSchema = await schema.resolve();
    const result = await safeParseAsync(typeof resolvedSchema === "function" ? resolvedSchema() : resolvedSchema, values, {
      abortPipeEarly: true
    });
    const formErrors = {};
    if (result.issues) {
...
const valiForm$ = implicit$FirstArg(valiFormQrl);
function getElementInput(element, field, type) {
  ...
  return !type || type === "string" ? value2 : type === "string[]" ? options ? [
    ...
    ...field.value || [],
    value2
  ] : (field.value || []).filter((v) => v !== value2) : type === "number" ? valueAsNumber : type === "boolean" ? checked : type === "File" && files ? noSerialize(files[0]) : type === "File[]" && files ? [
    ...files
...
const zodField$ = implicit$FirstArg(zodFieldQrl);
...
const zodForm$ = implicit$FirstArg(zodFormQrl);
...
function toCustomQrl(action, { on: mode }) {
  return $((value2, event, element) => event.type === mode ? action(value2, event, element) : value2);
}
...
const custom$ = implicit$FirstArg(customQrl);
