import { component$, useSignal, $ } from "@builder.io/qwik";

type Network = "xrpl" | "xahau";

const NETWORKS = {
  xrpl: {
    label: "XRPL",
    ws: "wss://xrplcluster.com",
  },
  xahau: {
    label: "Xahau",
    ws: "wss://xahau.network",
  },
};

export default component$(() => {
  const network = useSignal<Network>("xrpl");
  const wsUrl = useSignal<string>(NETWORKS.xrpl.ws);

  const toggle$ = $(() => {
    const next = network.value === "xrpl" ? "xahau" : "xrpl";
    network.value = next;
    wsUrl.value = NETWORKS[next].ws;
  });

  return (
    <div class="flex items-center gap-4 rounded-2xl bg-transparent px-4 py-2 shadow-md">
      {/* Toggle */}
      <label class="relative inline-flex cursor-pointer items-center">
        <input
          type="checkbox"
          class="sr-only peer"
          checked={network.value === "xahau"}
          onChange$={toggle$}
        />

        <div class="group relative h-10 w-20 rounded-full bg-purple-500 shadow ring-0 duration-300 peer-checked:bg-yellow-400 after:absolute after:left-1 after:top-1 after:flex after:h-8 after:w-8 after:items-center after:justify-center after:rounded-full after:bg-gray-50 after:content-[''] after:duration-300 peer-hover:after:scale-95 peer-checked:after:translate-x-10">
          {/* Xahau */}
          <svg
            class="absolute right-1 top-1 h-8 w-8 stroke-gray-900"
            viewBox="0 0 100 100"
          >
            <path d="M50,18A19.9,19.9,0,0,0,30,38v8a8,8,0,0,0-8,8V74a8,8,0,0,0,8,8H70a8,8,0,0,0,8-8V54a8,8,0,0,0-8-8H38V38a12,12,0,0,1,23.6-3,4,4,0,1,0,7.8-2A20.1,20.1,0,0,0,50,18Z" />
          </svg>

          {/* XRPL */}
          <svg
            class="absolute left-1 top-1 h-8 w-8 stroke-gray-900"
            viewBox="0 0 100 100"
          >
            <path
              fill-rule="evenodd"
              d="M30,46V38a20,20,0,0,1,40,0v8a8,8,0,0,1,8,8V74a8,8,0,0,1-8,8H30a8,8,0,0,1-8-8V54A8,8,0,0,1,30,46Zm32-8v8H38V38a12,12,0,0,1,24,0Z"
            />
          </svg>
        </div>
      </label>

      {/* Divider */}
      <div class="h-8 w-px bg-neutral-800" />

      {/* Network Info */}
      <div class="flex flex-col text-xs leading-tight">
        <span class="font-semibold text-white">
          {network.value.toUpperCase()}
        </span>
        <code class="mt-0.5 rounded bg-neutral-900 px-1.5 py-0.5 text-[10px] text-emerald-400">
          {wsUrl.value}
        </code>
      </div>
    </div>
  );
});
